...
<head>
  ...
  <title>Kelola Kepala Bidang</title>
  <!--     Fonts and icons     -->
      ...
      <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        <nav>
          ...
          <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            ...
            <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']" aria-current="page">Kelola Kepala Bidang</li>
          </ol>
          <h6 class="mb-0 font-bold text-white capitalize">Kelola Kepala Bidang</h6>
        </nav>
      ...
      <div class="overflow-x-auto bg-white shadow-lg rounded-xl p-4">
        <table class="min-w-full table-auto">
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            @foreach($kepalaBidang as $index => $kb)


            <tr class="hover:bg-blue-50">
              <td class="py-3 px-4 text-xs">{{ $index + 1 }}</td>
              <td class="py-3 px-4 text-xs font-medium">{{ $kb->name }}</td>
              <td class="py-3 px-4 text-xs text-center">
                <span class="inline-block px-2 py-1 text-xs rounded">{{ $kb->identitasTimAdmin->pangkat ?? '-' }}</span>
              </td>
              <td class="py-3 px-4 text-xs text-center">{{ $kb->email }}</td>
              <td class="py-3 px-4 text-xs font-medium text-center">Kepala Bidang</td>
              <td class="py-3 px-4 text-center">
                <form action="{{ route('kepala-bidang.destroy', $kb->id) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
          ...
          <tbody class="divide-y divide-gray-200">
            <div class="flex justify-between items-center mb-4">
              <h2 class="text-xl font-bold text-gray-700">Kelola Kepala Bidang</h2>
              <!-- Tombol di atas tabel -->
              <button onclick="openTambahTimteknisModal()"
                class="bg-blue-500 text-white px-4 py-2 font-bold text-sm rounded-lg hover:bg-blue-600 transition">
            ...
            <tr class="hover:bg-blue-50">
              ...
              <td class="py-3 px-4 text-xs text-center">
                <span id="pangkat_1" class="inline-block px-2 py-1 text-xs text-xs rounded">Pengatur Muda (II/a)</span>
              </td>
              <td class="py-3 px-4 text-xs text-center">budi.santoso@email.com</td> <!-- Email ditambahkan 
              <td class="py-3 px-4 text-xs font-medium text-center" id="peran_1">Kepala Bidang</td>
              <td class="py-3 px-4 text-center">
            ...
            <!-- Modal Popup Form Tambah Timteknis -->
            <div id="modalTambahTimteknis" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50 z-50">
              <div class="bg-white w-full max-w-md p-6 rounded-lg shadow-lg">
                <h2 class="text-lg font-semibold mb-4">Form Tambah Kepala Bidang</h2>
                <form action="{{ route('kepala-bidang.store') }}" method="POST">
                  @csrf
                  <div class="mb-3">
                ...
                <!--
                  <div>
                    <label for="pangkat" class="block mb-1 text-sm font-medium">Pangkat / Golongan</label>
                  ...
                  <div class="mb-3">
                    ...
                    <input type="email" name="email" id="email" required class="w-full border border-gray-300 px-3 py-2 rounded" />
                  </div>
                  <div class="mb-3">
                    <label class="block mb-1 text-sm font-medium">Kata Sandi</label>
                    <input type="password" name="password" id="katasandi" required class="w-full border border-gray-300 px-3 py-2 rounded" />
                  </div>
                  <div class="flex justify-end gap-2">
                    <button type="button" onclick="closeTambahTimteknisModal()" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Batal</button>
            ...
            <div id="popup-konfirmasi-hapus" class="fixed inset-0 hidden z-[9999] bg-black bg-opacity-50 flex items-center justify-center">
              <div class="bg-white rounded-lg p-6 w-80 shadow-lg text-center">
                <h2 class="text-lg font-semibold mb-4">Konfirmasi Hapus</h2>
                <p class="text-sm text-gray-700 mb-6">Apakah Anda yakin ingin menghapus Tim Teknis ini?</p>
                <div class="flex justify-center gap-4">
                  <button onclick="hapusTimteknis()" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">Hapus</button>
            ...
            <script>
              ...
              function openTambahTimteknisModal() {
                document.getElementById("modalTambahTimteknis").classList.remove("hidden");
                document.getElementById("modalTambahTimteknis").classList.add("flex");
              }

              function closeTambahTimteknisModal() {
                document.getElementById("modalTambahTimteknis").classList.remove("flex");
                document.getElementById("modalTambahTimteknis").classList.add("hidden");
                document.getElementById("formTimteknis").reset();
              }
              ...
              function submitFormTimteknis(event) {

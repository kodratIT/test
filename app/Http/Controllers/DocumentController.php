...
class DocumentController extends Controller
{
    ...
    private function generateSimpleDocument($pengajuan)
    {
        ...
        // 🔹 Placeholder, bukan isi otomatis
        $nomorDraft   = '${nomor_naskah}';
        $tanggalDraft = '${tanggal_naskah}';
        $tteDraft     = '${ttd_pengirim}';
    ...
    private function createWordDocument($pengajuan)
    {
        ...
        $phpWord->addParagraphStyle('tight_center', [
            'alignment' => Jc::CENTER,
            'spaceAfter' => 0
        ]);
        ...
        $section->addText('PERIODE TAHUN ' . date('Y'), 'teks', [
            'alignment' => Jc::CENTER,
            'spaceAfter' => 300 // hanya baris terakhir kasih jarak ekstra
        ]);

        // Nomor → pakai placeholder
        $section->addText(
            'Nomor ${nomor_naskah}',
            ['name' => 'Arial', 'size' => 14, '' => true],
            ['alignment' => Jc::CENTER, 'spaceAfter' => 400]
        );

        // Isi
        $section->addText('Diberikan kepada', 'isi', ['alignment' => Jc::CENTER, 'spaceAfter' => 200]);
        ...
        $section->addText($namaPerusahaan, 'nama', ['alignment' => Jc::CENTER, 'spaceAfter' => 100]);

        // Info perusahaan
        $nib = $identitas->nib ?? '-';
        $alamatKantorPusat = $identitas->alamatkantorpusat ?? 'Alamat tidak tersedia';
        $section->addText('NIB: ' . $nib, 'isi', ['alignment' => Jc::CENTER, 'spaceAfter' => 10]);
        $section->addText($alamatKantorPusat, 'isi', ['alignment' => Jc::CENTER, 'spaceAfter' => 10]);
        ...
        // === Tabel untuk TTD Pengirim & TTE Pejabat (side by side)
        ...
        // === Kolom kanan: TTE Pejabat
        $cell2 = $table->addCell(5500, ['borderSize' => 6]);
        $textRun2 = $cell2->addTextRun(['alignment' => Jc::START]);

        $textRun2->addText('Jambi, ${tanggal_naskah}', 'normal');
    ...
    private function addOfficialHeader($section)
    {
        ...
        $section->addText(
            'Jl. Jenderal Sudirman No. 1, Jambi 36122',
            'content_normal',
            ['alignment' => Jc::CENTER, 'spaceAfter' => 60]
        );
        $section->addText(
            'Telp. (0741) 61234, Fax. (0741) 61235',
            'content_normal',
            ['alignment' => Jc::CENTER, 'spaceAfter' => 60]
        );
        $section->addText(
            'Email: esdm@jambiprov.go.id | Website: www.jambiprov.go.id',
            'content_normal',
